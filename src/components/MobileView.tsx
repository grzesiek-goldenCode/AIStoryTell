...
export default function MobileView({ story, title }: StoryProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center ">
      <h2 className="mt-3 mb-5 text-4xl font-bold text-center">{title}</h2>
            ...
            {story.map((fragment, i) => (
              <SwiperSlide key={i} className="h-full w-full ">
                {({ isActive }) => (
                  ...
                  >
                    <div
                      className={`flex-1 text-4xl pt-3 h-full mb-0 overflow-y-auto scrollbar-hidden story-container${
